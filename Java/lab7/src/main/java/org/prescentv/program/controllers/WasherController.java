...
public class WasherController {
    ...
    void initializeCleanButton(ActionEvent event) {
        if (model.app.getOi() == 1) {
            if (((Washer)model.app).getLoaded() == 0 && ((Washer)model.app).getStart() == 0) {
                initializeCleanButton.setText("OUT OF RES");
                return;
            }
            if (((Washer)model.app).getStatus() == 2) {
                if (((Washer)model.app).startCleaning(System.currentTimeMillis()/1000) == 0) {
                    initializeCleanButton.setText("INITIALIZE CLEAN");
                    currentResLabel.setText(Integer.toString(((Washer)model.app).getLoaded()));
                    String[] statuses = ((Washer) model.app).getStatuses();
                    currentStatusLabel.setText(statuses[((Washer) model.app).getStatus()]);
                    return;
                }
            }
            if (((Washer)model.app).startCleaning(System.currentTimeMillis() / 1000) == 2) {
    ...
    void toggleDevice(ActionEvent event) {
        if (model.app.getOi() == 0) {
            ...
            String[] statuses = ((Washer) model.app).getStatuses();
            currentStatusLabel.setText(statuses[((Washer) model.app).getStatus()]);
            String[] modes = ((Washer) model.app).getModes();
            currentModeLabel.setText(modes[((Washer) model.app).getMode()]);
            currentResLabel.setText(Integer.toString(((Washer)model.app).getLoaded()));
    ...
    void initialize() {
        ...
        modeList = FXCollections.observableArrayList(Arrays.asList(((Washer) model.app).getModes()));
        ...
        if (model.app.getOi() == 1) {
            ...
            currentModeLabel.setText(modes[((Washer) model.app).getMode()]);
            currentResLabel.setText(Integer.toString(((Washer)model.app).getLoaded()));
            modeChoiceBox.setValue(modes[((Washer) model.app).getMode()]);
        } else {
            toggleLabel.setText("TURNED OFF");
            currentStatusLabel.setText("");
            currentModeLabel.setText("");
    ...
    void setModeChoiceBoxListener() {
        modeChoiceBox.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observableValue, Number number, Number number2) {
                if (model.app.getOi() == 1 && ((Washer)model.app).getStatus() == 0) {
                    ((Washer) model.app).changeMode((Integer) number2 + 1);
